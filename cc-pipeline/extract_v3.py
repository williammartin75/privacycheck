#!/usr/bin/env python3
"""
extract_v3.py — Optimized Common Crawl WET Extractor (v3)
5 optimizations for ~2.5x speedup over v2:
  1. 2 parallel workers per VPS (multiprocessing via separate invocations)
  2. Pipeline download (background thread downloads while main processes)
  3. zcat subprocess instead of Python gzip (native C decompression)
  4. Quick-reject: skip WARC records without '@' in content
  5. Optimized classify: single text.lower() + 'in' instead of 'count'

Usage:
  python3 extract_v3.py 0 2   # Worker 0 of 2
  python3 extract_v3.py 1 2   # Worker 1 of 2
"""
...
# ══════════════════════════════════════════════════════════════
# INDUSTRY SECTOR CLASSIFICATION (optimized: 'in' instead of 'count')
# ══════════════════════════════════════════════════════════════
...
def classify_industry(text_lower):
    """Score text against industry keywords. Uses 'in' for speed."""
    scores = {}
...
# ══════════════════════════════════════════════════════════════
# OPTIMIZED WET FILE PROCESSING (zcat + streaming + quick-reject)
# ══════════════════════════════════════════════════════════════
    ...
    try:
        ...
        for raw_line in proc.stdout:
            ...
            if line.startswith('WARC/1.'):
                # Process previous record — only if emails were found
            ...
            if in_content:
                # Quick-reject: only scan for emails if @ present
                if '@' in line:
...
def main():
    ...
    # Resume support (shared progress file — atomic line writes)
    done_set = set()
    ...
    # Start pipeline download thread (pre-downloads up to 2 files ahead)
    ...
    while True:
        ...
        filename = os.path.basename(wet_path)
        print(f"[Worker {worker_id}] [{done}/{total}] {filename}")
