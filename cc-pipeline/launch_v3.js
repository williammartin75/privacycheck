#!/usr/bin/env node
/**
 * launch_v3.js — Stop v2, deploy extract_v3.py, launch 2 workers per VPS
 */
    ...
    return new Promise(resolve => {
        ...
        c.on('ready', () => {
            c.exec(cmd, { pty: false }, (err, stream) => {
                ...
                stream.on('close', code => { clearTimeout(timer); c.end(); resolve({ ok: true, out: out.trim(), code }); });
            });
        });
...
function scpUpload(host, pass, localPath, remotePath, timeout = 60000) {
    return new Promise(resolve => {
        const c = new Client();
        const t = setTimeout(() => { c.end(); resolve({ ok: false }) }, timeout);
        c.on('ready', () => {
            c.sftp((err, sftp) => {
                if (err) { clearTimeout(t); c.end(); return resolve({ ok: false }) }
                const rs = fs.createReadStream(localPath);
                const ws = sftp.createWriteStream(remotePath);
                ws.on('close', () => { clearTimeout(t); c.end(); resolve({ ok: true }) });
                ws.on('error', () => { clearTimeout(t); c.end(); resolve({ ok: false }) });
                rs.pipe(ws);
            });
        });
        c.on('error', () => { clearTimeout(t); resolve({ ok: false }) });
        c.connect({
            host, port: 22, username: 'root', password: pass, readyTimeout: 10000,
            algorithms: { kex: ['ecdh-sha2-nistp256', 'diffie-hellman-group14-sha256', 'diffie-hellman-group14-sha1'] }
        });
...
const BATCH = 10;
const EXTRACT_V3 = path.join(__dirname, 'extract_v3.py');

(async () => {
    // ── Step 1: Stop v2 + clean ──
    console.log('=== Step 1: Stop v2 + clean old results ===\n');
    for (let i = 0; i < ALL_VPS.length; i += BATCH) {
        ...
        const results = await Promise.all(batch.map(vps =>
            sshExec(vps.ip, vps.pass,
                'pkill -9 -f "extract\\.py" 2>/dev/null; pkill -9 -f "extract_v3" 2>/dev/null; ' +
                'rm -f /root/cc_results.ndjson /root/cc_progress.log /root/cc_extract*.log /tmp/w*.gz 2>/dev/null; ' +
                'echo OK', 15000)
        ));
        results.forEach((r, j) => console.log(`  VPS-${String(batch[j].id).padStart(2)}: ${r.ok ? '✅' : '❌ ' + r.out}`));
    }

    // ── Step 2: Upload extract_v3.py ──
    console.log('\n=== Step 2: Deploy extract_v3.py ===\n');
    let deployed = 0;
    for (let i = 0; i < ALL_VPS.length; i += BATCH) {
        const batch = ALL_VPS.slice(i, i + BATCH);
        const results = await Promise.all(batch.map(vps =>
            scpUpload(vps.ip, vps.pass, EXTRACT_V3, '/root/extract_v3.py', 30000)
        ));
        results.forEach((r, j) => {
            console.log(`  VPS-${String(batch[j].id).padStart(2)}: ${r.ok ? '✅' : '❌'}`);
            if (r.ok) deployed++;
        });
    }
    console.log(`\n  Deployed: ${deployed}/40\n`);

    // ── Step 3: Launch 2 workers per VPS ──
    console.log('=== Step 3: Launch 2 workers per VPS ===\n');
    let launched = 0;
    for (let i = 0; i < ALL_VPS.length; i += BATCH) {
        const batch = ALL_VPS.slice(i, i + BATCH);
        const results = await Promise.all(batch.map(vps =>
            sshExec(vps.ip, vps.pass,
                // Launch worker 0 and worker 1
                'nohup python3 /root/extract_v3.py 0 2 > /root/cc_extract_0.log 2>&1 & disown; ' +
                'nohup python3 /root/extract_v3.py 1 2 > /root/cc_extract_1.log 2>&1 & disown; ' +
                'sleep 3; pgrep -c -f extract_v3 2>/dev/null || echo 0',
                20000)
        ));
        results.forEach((r, j) => {
            const count = parseInt(r.out) || 0;
            console.log(`  VPS-${String(batch[j].id).padStart(2)}: ${count >= 2 ? '🚀 ' + count + ' workers' : count === 1 ? '⚠️ 1 worker' : '❌'}`);
            if (count >= 1) launched++;
        });
    }

    // ── Step 4: Verify ──
    console.log(`\n=== Step 4: Verify (5s later) ===\n`);
    await new Promise(r => setTimeout(r, 5000));
    let running = 0;
    for (let i = 0; i < ALL_VPS.length; i += BATCH) {
        const batch = ALL_VPS.slice(i, i + BATCH);
        const results = await Promise.all(batch.map(vps =>
            sshExec(vps.ip, vps.pass, 'pgrep -c -f extract_v3 2>/dev/null || echo 0', 10000)
        ));
        results.forEach((r, j) => {
            const count = parseInt(r.out) || 0;
            if (count >= 2) running++;
            console.log(`  VPS-${String(batch[j].id).padStart(2)}: ${count >= 2 ? '✅ ' + count + ' workers' : count === 1 ? '⚠️ 1 worker' : '⏸️ 0'}`);
        });
    }

    console.log(`\n✅ ${running}/40 VPS running 2 workers each (${running * 2} total workers)`);
